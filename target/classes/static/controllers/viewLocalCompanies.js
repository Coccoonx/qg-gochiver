angular
		...
		.controller(
				...
				function($scope, $http, $interval, uiGridConstants, $location,
					...
					$scope.getAllLocalCompanies = function() {
					...
					$scope.getAllLocalCompanies();
					
					$scope.gridOptions.columnDefs = [ {
						name : 'id',
						displayName : 'LocalCompany Id',
						enableHiding : false,
						enableCellEdit : true
					...
					$scope.addLocalCompany = function() {
						$location.path('/addLocalCompany');
					}
					
					$scope.deleteLocalCompany = function() {
						
						var row = $scope.gridApi.selection.getSelectedRows();
						$localStorage.companyId = row[0].id;
						console.log($localStorage.companyId);
						
						swal(
								{
									title : "<b> Delete Local Company </b>",
									text : "Would you like to delete this local Company?",
										...
										$http(
												{
													...
													console.log("LocalCompany deleted sucessfully");
									       			$auth.setToken($localStorage.token);
									       			swal("Success!",
															"Local company deleted successfully.",
															"success");
									       		})
												.error(
														function(error) {
															$auth.setToken($localStorage.token);
															toastr
																	.error("Failed to delete localcompany '"
